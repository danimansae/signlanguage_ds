...
public class MyServie extends RecognitionService {
    ...
    boolean end = false;
//RecognitionService는 안드로이드 Service 라이브러리 중 하나로 핸드폰 기기의 음성을 받아들이는 서비스
 ...
 */
    ...
    private RecognitionListener mClsRecoListener = new RecognitionListener() {
        @Override
        public void onRmsChanged(float rmsdB) { //오디오 스트림의 사운드 레벨이 변경
            Log.d("sound",""+rmsdB+"dB");
            //입력되는 데시벨 크기를 상수로
            //들어오는 입력을 UI에서 다이나믹에서 보여줄 때 사용
            if(rmsdB>30)
            {
                Toast.makeText(getApplicationContext(), "큰소리 발생", Toast.LENGTH_SHORT).show();
            }
        }
